...
\setbookmaintitle{The \env{musicexamples} LaTeX{} Package}
\author{Urs Liska}
...
Usually it is enough to copy it somewhere into your \texttt{texmf} tree and run \env{texhash} afterwards.
...
\#2: the caption and \#3: a \cmd{label}.
...
This way you actually have a \cmd{label} at the right place in your source, and your editor may use this in helping you navigate your document.
...
\cmd{fullPageLilypondExample} behaves the same as \cmd{fullPageMusicExample} but you should use it for examples created with LilyPond, as it can be used (e.\,g. by scripts) to keep track of these examples (see \fref{sec:xmp_python}).
...
If you give a filename that doesn't point to an existing and valid image file two things will happen:
...
More optional floating environments that are available when third-party packages are loaded (some comments earlier in \fref{subsec:xmp_basic-usage}, more information in the respective package documentation):
\begin{description}
...
\begin{description}
\item[\cmd{musicSFE} and \cmd{lilypondSFE}]Print a single file.
