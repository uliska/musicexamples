...
Internally it includes the pages using \cmd{includepdf} and the \package{afterpage} package.
You have to specify the file name of the score (including the relative path) as mandatory argument \#1.
As it is a command and not an environment you have to provide a caption and a label as mandatory arguments (\#2 and \#3) too.
...
The pagestyle for the first page is set to \env{xmpPageOne} while the remaining pages are assigned \env{xmpPageRest}.
