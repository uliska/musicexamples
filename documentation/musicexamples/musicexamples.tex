...
\label{subsec:basic-usage_environments}
...
\label{subsec:basic-usage_full-page-examples}
...
The command checks the existence of a pdf file with the basename given as the first argument, but also of files with \texttt{-odd} or \texttt{-even} suffixes.
...
If you finally have a two-sided score that should explicitely start on an odd or even page (typically one might want to have two page scores printed on one double page) then you should only provide one file with the appropriate suffix, which will be printed on the next odd or even page (as indicated by the file suffix), possibly skipping another page of text.
...
\begin{knownIssues}
The LilyPond configuration files provide commands to compile odd- and even-starting pdf files from one source file, but that is not quite perfect and not documented yet. 
...
Other than described above the functionality to explicitely start on an odd or even page isn't working so far!
Obviously I've run into an issue with macro expanding that I didn't manage to solve so far \ghIssue{23}
\end{knownIssues}
