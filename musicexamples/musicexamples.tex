...
Internally the pages are included using \cmd{includepdf} and the \package{afterpage} package, so the score will start at the next page without leaving the rest of the page empty.
...
The pagestyle for the first page is set to the predefined \env{xmpPageOne} while the remaining pages are assigned \env{xmpPageRest}.
...
So far the score will start on the \emph{next} page, but it is intended to implement an option to start at the next odd or even page \ghIssue{10}.
\end{knownIssues}
